...
| `CONVENTIONS.md` | This file. Lives at the consuming repo's root as a copy of the plugin's canonical version, written by `/genvid:audit-conventions --fix` on first migration. The audit reports drift on each run so you can re-sync after the plugin updates. | Yes |
...
> **Scope note:** genvid carries project-aware workflows, not generic tooling. It deliberately does not reimplement standalone PR review or code simplification — use Anthropic's official `code-review` (`/code-review`) and `code-simplifier` plugins for those. The `genvid:code-reviewer` agent exists only as the `plan-task` review gate.
...
|---------|---------|------------------------|
| `Commit Format` | `/genvid:commit-changes` | The exact commit message format your team uses (subject line shape, body, trailers). |
| `Pull Request Format` | `/genvid:create-pr` | PR title and body conventions; whether you use Bitbucket or GitHub if non-obvious from the git remote. |
| `Branching` | `/genvid:rebase-branch`, `/genvid:split-branch` | Branch naming, base branch, when to rebase vs. merge. |
...
- `test`, `lint`, `build`, `validate`: required if the corresponding skill is used. `/genvid:validate-changes` reads `commands.validate`.
...
## How `/genvid:audit-conventions` works
