...
> **`.gitignore` note for `search-docs`:** The shipped `.gitignore` covers `extracted/c3-reference/` (the default). If you set `extractedDir` to a custom value (e.g. `"c3-extracted"`), add `c3-extracted/c3-reference/` to your `.gitignore` — the reference cache is large and regenerated by the genvid-c3 `build-reference` skill, not by `generate`.
