...
- **File-system watcher + self-write suppression.** `fs.watch` on the source dirs detects external changes (editor saves, user edits) and bumps `txId` + sets `extractedDirty`. To avoid double-counting the server's own writes, write tools register each path in an `ExpectedChanges` set (TTL-based, periodically purged) and bump a `suppressWatcherDepth` counter around the write; the watcher skips paths it expected. **Editing a mutate tool means you must register expected writes and bump the suppress depth, or the watcher will spuriously mark state dirty.**
