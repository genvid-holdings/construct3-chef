...
- **Response shape: two content blocks (`text` + `txId`), kept local.** Mutate/validate/sync tools return *two* content blocks — the result/error text, then a separate `txId: <n>` block — on both success and error paths (error/rejection blocks add `isError: true` and an `Error:` prefix; a few txId-less tools like `regenerate`/`generate-sids`/`notFound` return a single block). The repeated error/rejection form is deduped behind local `errorWithTxId(message)` / `caughtError(e)` helpers in `server.ts`. This shape is **deliberately not** delegated to `@genvid/mcp-utils`' `mcpError`/`withMcpErrors`: those collapse to a *single* joined block, drop the `Error:` prefix, and can't model the txId-as-second-block convention — which is *this server's* optimistic-concurrency shaping, not generic MCP plumbing — and `withMcpErrors` can't host the `CancelledError → watcher.bump()` side-effect catches. Per the adoption posture (generic plumbing upstream, project-specific response shaping local), the error/txId shaping stays local. (Settled in [#25](https://github.com/genvid-holdings/construct3-chef/issues/25), PR #70.) **Pagination took the *opposite* call in [#26](https://github.com/genvid-holdings/construct3-chef/issues/26):** `paginatedResponse` now *adopts* upstream `paginatedContent`, returning the page text and the `lines: X-Y / total` range footer as a **single** block (the stale-warning is folded into that block via the unchanged `appendStaleWarning`, since it carries no upstream equivalent; an out-of-range page now reports the honest `lines: 0 / total` rather than the old misleading `lines: <offset>-<offset>`). The client-visible contract-break was accepted as fine while the package is young — so the error/txId two-block shaping above is now the *remaining* local holdout, and adopting upstream there is blocked until `@genvid/mcp-utils` grows a `withMcpErrors(fn, { onError })` side-effect hook (to host the `CancelledError → watcher.bump()` catch), a structured single-block footer so `txId` can ride along, and a configurable `Error:` prefix (an intent request tracked from [#25](https://github.com/genvid-holdings/construct3-chef/issues/25)). Both contracts are guarded by handler-level characterization tests (`test/mcp/serverHandlers.test.ts`), which invoke registered handlers directly via an internal `reg()` / `__getHandler` seam in `server.ts` against a fixture with a fake watcher — exercising the txId-rejection, stale-warning, single-block pagination ([#26](https://github.com/genvid-holdings/construct3-chef/issues/26)), and `CancelledError`-ordering paths ([#72](https://github.com/genvid-holdings/construct3-chef/issues/72), PR #73).
