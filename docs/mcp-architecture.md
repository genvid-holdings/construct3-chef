...
- **Response shape: two content blocks (`text` + `txId`), kept local.** Mutate/validate/sync tools return *two* content blocks — the result/error text, then a separate `txId: <n>` block — on both success and error paths (error/rejection blocks add `isError: true` and an `Error:` prefix; a few txId-less tools like `regenerate`/`generate-sids`/`notFound` return a single block). The repeated error/rejection form is deduped behind local `errorWithTxId(message)` / `caughtError(e)` helpers in `server.ts`. This shape is **deliberately not** delegated to `@genvid/mcp-utils`' `mcpError`/`withMcpErrors`: those collapse to a *single* joined block, drop the `Error:` prefix, and can't model the txId-as-second-block convention — which is *this server's* optimistic-concurrency shaping, not generic MCP plumbing — and `withMcpErrors` can't host the `CancelledError → watcher.bump()` side-effect catches. Per the adoption posture (generic plumbing upstream, project-specific response shaping local), it stays local. The same call governs `paginatedResponse`'s `text` + `lines: X-Y` two-block shape vs upstream `paginatedContent` ([#26](https://github.com/genvid-holdings/construct3-chef/issues/26)). (Settled in [#25](https://github.com/genvid-holdings/construct3-chef/issues/25), PR #70.) This contract is now guarded by handler-level characterization tests (`test/mcp/serverHandlers.test.ts`), which invoke registered handlers directly via an internal `reg()` / `__getHandler` seam in `server.ts` against a fixture with a fake watcher — exercising the txId-rejection, stale-warning, and `CancelledError`-ordering paths ([#72](https://github.com/genvid-holdings/construct3-chef/issues/72), PR #73).
