...
### ~~Gap: `grep` filter only matches event descriptions~~ ✅ Fixed (2026-05-28)

`SidMapEntry` (in `src/c3/dslFormatter.ts`) now carries a `searchText` field alongside `description` — a newline-joined summary of the event's own conditions (via `formatConditionWithDisabled` → c3source `formatCondition`, includes parameter pairs and `NOT` / `[DISABLED]` prefixes) and actions (via c3source `formatAction`, includes parameter values, `[behaviorType]` segments, `[DISABLED]` prefixes, and full untruncated comment/script text) for `block` / `function-block` / `custom-ace-block`, empty otherwise. The `read-event-sids` MCP handler greps over both fields, so all the original failing queries — `grep=BattleLayout` (an action parameter value), `grep=on-touched-object`, `grep=GoToLayout`, `grep=[DISABLED]`, `grep=NOT` — now find the relevant blocks. The shared `EventCounter` in `buildShallowSidMap` is incremented for every counter-bearing event type (group, block, function-block, custom-ace-block) in lockstep with `formatBlockLike`, so the synthetic script-function names baked into `searchText` (e.g. `Sheet_Event2_Act1`) match what `extractScriptsFromSheet` emits and round-trip via `extracted/scripts/*.ts`. Defensive `?? []` guards on `event.conditions` / `event.actions` keep the handler resilient against legacy or hand-edited JSON that omits an array. Displayed output is unchanged (`searchText` is never rendered). 11 new/revised tests in `dslFormatter.test.ts` cover condition/action match, `NOT` prefix, `[DISABLED]` on both conditions and actions, `[behaviorType]`, action parameter values (the original gap query), function-block inner actions, empty `searchText` for variable/include/comment/group, missing-array defensive guard, and the `group → block` counter alignment. **Known scope-limit:** searchText is shallow per-event — a grep match in a nested child returns the child's SID, not the enclosing block's; the tool description advertises this.

_Original report:_ The `grep` parameter filtered against the `description` column (comment text, function names), but not action content (function call targets, parameter values, object classes). When looking for the SID of a block that called `GoToLayout("BattleLayout")` or had a condition on `end-game-continue`, the grep returned no matches — forcing fallback to raw `Grep` on the JSON file and manual SID extraction. Observed in the story battle menu session: needed SID of the `else` block calling `GoToLayout("BattleLayout")` in ModalEvents; `grep=BattleLayout`, `grep=else`, `grep=end-game-continue` all returned nothing.

### Gap: `read-dsl-index` has the same condition/action content limitation

The `read-event-sids` fix above broadens grep over `block` / `function-block` / `custom-ace-block` summaries, but `read-dsl-index` (which `filterIndex`-greps over `.dsl.idx.txt`) is unchanged and still suffers the same shape. `formatBlockLike` pushes a `DslIndexEntry` per action (carrying `describeAction` text) but **no row per condition**, so `read-dsl-index grep=on-touched-object` cannot match a condition row that doesn't exist in the index. Worse, action rows are still produced via `describeAction`, which drops parameter values, `[behaviorType]`, and `[DISABLED]` — so `read-dsl-index grep=BattleLayout` keeps failing exactly where `read-event-sids grep=BattleLayout` now succeeds.

**Proposal:** Push a `condition[N]` `DslIndexEntry` row in `formatBlockLike` right after each `when:` line (mirroring the existing per-action row). Either widen `describeAction` to include parameter values / `[behaviorType]` / `[DISABLED]` (and accept the slightly longer index rows), or have `formatBlockLike` use `formatAction` directly for its index-entry descriptions. The cleanest version factors `summarize` in `buildShallowSidMap` and the per-condition / per-action `indexEntries.push` loop in `formatBlockLike` into a single helper so the two surfaces can't drift again.

**Observed in:** the same code-review pass that produced the `read-event-sids` fix (2026-05-28). The MCP `search type=dsl` tool already covers the underlying content (it greps the rendered `.dsl.txt`) — but it returns file/line hits, not SIDs, so users still come back to `read-dsl-index` / `read-event-sids` for SID coordinates.

### Gap: `read-event-sids` matched rows give no signal of *what* matched

When `grep` hits via the new `searchText` field, the rendered table row still shows only the unchanged `description` column (typically just `block` / `block [OR]` / `function "name"`). The caller sees a SID but not the condition or action that triggered the match, so the typical workflow becomes `read-event-sids grep=X` → got 3 SIDs → `read-dsl` each of them to figure out which is the right one. This partially undoes the per-call efficiency gain of having the grep filter at all.

**Proposal:** When filtering, render an additional indented line per row showing the first `searchText` line that the regex matched — e.g. `  ↳ matched: System.go-to-layout(layout=BattleLayout)`. Cheap, keeps the pipe-delimited header row format stable for clients that parse it, and surfaces the actual hit. Skip the extra line for rows that matched only via `description` to avoid clutter.

**Observed in:** same code-review pass (2026-05-28). Low priority — `read-dsl` is a cheap follow-up — but worth doing when the next round of `read-event-sids` work happens.
