...
User-defined ops (#21 / #23 phase 3) add the one piece of *dynamic* tool surface to an otherwise static registry. `src/mcp/opsRegistry.ts`'s `OpsRegistry` loads parameterized recipe templates from a configurable directory (`ops.dir`, default `ops/`) and registers one `op-<name>` MCP tool per file plus a static `list-ops` tool. Three things about how it fits the concurrency model:

- **Its `fs.watch` is separate from the source `OptimisticWatcher`.** Op files are not C3 source, so edits to them must NOT bump `txId` or set `extractedDirty`. The registry runs its own debounced (~150 ms) `fs.watch` on `ops.dir` (gated by `ops.watch`, default `true`; set `false` to disable where `fs.watch` is costly — ops are then scanned once at startup and a change needs a restart). A watch event triggers `reconcile()`, which diffs loaded ops against the registered set and adds / `update()`s / `remove()`s tools accordingly. The op-file watch never touches the source-write `suppress`/`expect` machinery.
- **Op handlers route writes through the shared apply path.** Applying an op = substitute `{{PARAM}}`s → a `Recipe`, then call `applyRecipeWithConcurrency` — the same `rwlock.write` + `watcher.suppress` + `txId` bump + regenerate orchestration that backs `apply-recipe` (extracted into a shared helper for exactly this reuse). Op tools therefore inherit the full concurrency posture for free; only their *registration* is dynamic, not their write path.
- **`tools/list_changed` is SDK-driven.** The server declares `capabilities.tools.listChanged: true`; `@modelcontextprotocol/sdk`'s `registerTool` / `RegisteredTool.update()` / `.remove()` auto-emit `notifications/tools/list_changed` *only when connected* — so initial registration (before `server.connect`) is silent and post-connect reconciles notify automatically. The registry never calls `sendToolListChanged()` itself (that would double-notify).
